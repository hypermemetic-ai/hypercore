...
When the operator asks for a plan diagram, `bpmn-plans` preserves every work-specific action, decision, failure path, and acceptance Check as an explicit flow node and never simplifies or removes that content, then collapses inherited review/commit/push/PR/Checks mechanics—including Task finalization before the final handoff—into one `Complete qq Change delivery` call activity immediately before `Green PR ready`. Only operator disposition, merge, post-merge synchronization, and cleanup remain beyond that modeled boundary. Candidate generation is private: only the final stored, linked, verified plan is opened for the operator, exactly once when its approval question is ready; an unchanged version is not reopened (`skills/bpmn-plans/SKILL.md:1-11`, `22-41`, `71-93`; `skills/deliver-change/SKILL.md:54-59`).
