...
- progress (2026-06-13): the workspace is cloned and ready — plexus-trak (on its default branch feature/AUTHZ-TENANT-GATE-trak-facets, the legacy-HS256 single-user path) plus its four path deps plexus-core/-macros/-transport/-auth-core, all next to it in ~/projects. The toolchain wall is cleared: the operator decided option 1 (install rustup) and this session installed rustup current-stable into ~/.cargo + ~/.rustup — cargo/rustc 1.96.0 (stable) are usable. Toolchain card cleared from work.md in the same commit. The build now runs against the staged workspace; what remains is the cargo build, running trak on :44107 under legacy-HS256 single-user auth, and confirming a facet write round-trips.
- state: open
- since: 2026-06-13
- of: switch hypercore's document backend to trak
