...
Build note (the operator settled the prior crossing's depth decision — keep the docs): author the new scenario verbs (`fails`, `card`, `node`) in `engine/worlds/worker_world.py` WITH their full explanatory comments and docstrings, in that file's established style — do NOT strip or thin any existing comments to stay under the 400-line length signal. The file will exceed 400 lines; that is intended and already accepted. The length acceptance for `engine/worlds/worker_world.py` is already recorded in `engine/accepted-lengths.md` — do NOT modify that ledger and do NOT shorten the file to fit a line budget. The production fix is unchanged: `worker.run`'s own `except` raises the parented decision card before recovering, and the scheduler's now-redundant `_fail` copy is removed (`engine/worker.py`, `engine/schedule.py`).
