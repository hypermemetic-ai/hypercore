  ...
  "workspace list")
    if [ -n "${FAKE_CLEAN_WORKTREE:-}" ] && [ -d "$FAKE_CLEAN_WORKTREE" ]; then
      jq -cn --arg checkout "$FAKE_CLEAN_WORKTREE" '
        {result:{workspaces:[
          {workspace_id:"clean-ws",worktree:{checkout_path:$checkout,is_linked_worktree:true}}
        ]}}'
    else
      printf '%s\n' '{"result":{"workspaces":[]}}'
    fi
...
assert_file_contains "$scan_report" '"id":"branch:merged-feature"'
...
assert_not_contains "$(<"$scan_report")" 'branch:unmerged-feature' \
  'unmerged branch was nominated'
assert_not_contains "$(<"$scan_report")" 'branch:clean-wt' \
...
assert_not_contains "$(<"$scan_report")" 'branch:main' 'main was nominated'
...
sed -i 's/branch:merged-feature/unknown:merged-feature/g' "$unknown_report"
