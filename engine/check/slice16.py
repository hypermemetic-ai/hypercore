"""Slice 16 — the autonomy seam: the scheduler consumes the ready work.

Acceptance (spec/schedule, intent §60/§62/§110): a ratified ask is *built*, not left standing while
the system idles. `tree.ready` computed the ready work; this slice builds the loop that consumes it —
`worker.run` (dispatch → build fenced → integrate → fold), wired off the operator's path and driven by
the scheduler. The check drives the real scheduler, the real tree, real fences, and a real fold over
...
3. **readiness gates scheduling** — a standing node blocked on an open child is not in the ready work and
...
tree's ready work, so it must not reach into the sibling slices' leftover standing work.
"""
...
def check(_shared_root: str) -> None:
    from .. import tree, schedule, spec, worker

    print("\nslice 16 — acceptance check  (the autonomy seam: the scheduler runs the ready work)\n")

    root = tempfile.mkdtemp(prefix="engine-check-slice16-")
    os.environ["ENGINE_ROOT"] = root                 # isolate: the scheduler reads the whole ready work
    ...
    live = lambda: [n for n in tree.read_tree() if n.is_live]
    folded = lambda i: any(n.id == i and n.folded for n in tree.read_tree())
    ...
    a = tree.file_intent("schedule the ALPHAMARK work")
    b = tree.file_intent("schedule the BETAMARK work")
    ...
    ok(time.time() - t0 < 2.0 and len(sched.running) == 2,
       "step dispatches the ready work and returns at once — two workers run, off the operator's path")
    ok(wait_until(lambda: len(live()) == 2),
       "both ready nodes go in flight at the same time — the ready work is consumed concurrently (§62)")
    ...
    parent = tree.file_intent("a parent ask with a question beneath it")
    tree.raise_card("an open question only you can settle", kind="decide", parent=parent.id)
    ok(parent.id not in {n.id for n in tree.ready()},
       "a standing node blocked on an open child is absent from the ready work (§110)")
    ...
    good = tree.file_intent("schedule the GOODMARK work")
    bad = tree.file_intent("schedule the FAILMARK work")
    ...
    fail_cards = [n for n in tree.cards() if n.parent == bad.id and n.kind == "decide"]
