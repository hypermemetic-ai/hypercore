...
segment: the home mounts the operator's work folders and governs them within itself.
This node holds its own corpus, divided into one segment to start:

- **mounting** -- how a work folder mounts as a distinct git repository and child node, and
  how the recursive check reaches it.
...
`material/` -- the mount point each work folder mounts into as a child node of this home at
`material/<name>/`. The home holds zero work folders yet.
