...
Every operator word lands a concrete, findable consequence: a change, a decision returned to them, or a written reason nothing moved — and the record leads from the word to what was done about it. A word that produced none of these is a system failure, not a judgment call.
...
Brownfielding an external project is distillation, then ratification: the folder enters the registry, the machine drafts intent.md (machine-owned to the last word) from the project's own record — documents where they exist, else the code, tests, and history themselves — and the operator's picks decide what survives; the project keeps running on its old record until intent supplants it.
