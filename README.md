...
- `material/` — the code and child nodes that materialize the intent: the methodology prose (`hypercore.md`), `check.sh`, the adapter, active work nodes, and durable child nodes such as `material/home/`. `check.sh` re-runs the structural statements against the root and every child node.
