...
hypercore governs its own evolution. Work runs the loop — orient, frame, implement, check, archive. Active root-directed work lives directly under the root as `<NNN-slug>/`, with the written frame under `intent/frame/` inside that work node.
