...
- **a load-bearing interface decision is designed twice, in isolation** — A load-bearing interface decision MUST be designable as a contest of several candidates, one per brief (minimize the interface / maximize flexibility / optimize the common caller / ports-and-adapters), each running in its own fence — the worker's worktree, tagged per candidate — so the candidates advance the same decision at once, isolated from each other and the main line. The architect judges an interface load-bearing (ADR 0007); the depth gate's "cannot deepen in place" is the second entry (ADR 0006).
