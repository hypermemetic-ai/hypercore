...
- **the disciplines are smells a reader judges, not thresholds a tool measures** — The red flags MUST be carried as **judgment**, not numbers — shallow module, information leakage, temporal decomposition, overexposure, pass-through method, repetition, special-general mixture, conjoined methods, comment-repeats-code, vague or hard-to-pick name, nonobvious code — each a symptom a judge weighs, none a threshold a tool checks. The system keeps at most a **length** tripwire (a context-cost signal that raises a decision, never an auto-refusal); the model-driven red-flag scan lives in `architecture-review`, recorded as not-yet-built, never fabricated.
