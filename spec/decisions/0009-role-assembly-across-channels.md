...
`worker.DEPTH` — a hand-compression of `research/aposd.md` frozen in a Python constant, a second copy
...
- **The worker holds the whole spec, preloaded, by construction — the keystone.** Every episode the
  ...
  non-slice-confinement of rebuild-spec §4.1/§6.4 (already built in slice 4 and tested by `slice4`):
...
- **Single-sourcing is a derived render, regenerated by the fold.** Skills and the agents file are
  generated from the source (the depth skill and the worker's depth grounding from `research/aposd.md`;
...
twice), a tested keystone (§6.4), and the defense against both the architect's mis-scoping and worker
...
from `research/aposd.md`, retiring the frozen `worker.DEPTH`; the derived-render / materialize-on-fold
