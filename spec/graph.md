...
open graphs live under `work/` and folded graphs under `work/archive/` — the archive nested one
level down so the live work sits at the front of the tree. A graph's child graphs nest the same way
in its own `work/`. The on-disk unit is the graph, never the single node.
...
- THEN it is the folders under `work/` (its nested `archive/` included) read recursively, each one
  graph, with its operation kind, state, owner, and whether it carries the `[machine]` marker
  recovered from its `intent.md`

### Requirement: folding moves the graph's folder into work/archive/
A graph that folds MUST move from its `work/` into that work/'s own `archive/`; location is
authoritative, so a folded graph cannot sit directly in `work/`, nor an open one under `archive/`,
and the move carries the graph's material and child graphs whole.
