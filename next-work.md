...
> **Scaffold note (ADR 0010, 2026-06-21).** `rebuild-spec` was the bootstrap methodology doc
> (`~/Documents/rebuild-spec-1.md`), now **retired** — its content is absorbed into `intent.md`,
> the living spec, and the ADRs. The `rebuild-spec §N` references in the historical entries below
> are kept as written but point at no live doc; the in-repo home (a `spec/<cap>/spec.md` or an ADR)
> is the standing source now. The depth synthesis moved with it: `research/aposd.md` →
> `spec/depth.md`. This journal is itself working scaffold — the standing story lives in the README,
...
disciplines are no longer a frozen hand-compression of `spec/depth.md` in a Python constant —
...
- **The build sequence** (`research/assembly.md` §5), lowest-regret first, no harness seam needed:
  (1) retire `worker.DEPTH` — render the depth grounding from `spec/depth.md`, create the depth
...
- **Single-sourcing is a derived render, regenerated by the fold.** Skills + agents files are
  *generated* from the source (the `depth` skill from `spec/depth.md`; a per-capability skill
...
slices 1–8 pass** (`python3 -m hyper --check`, 107 checks, 0 fail). (rebuild-spec, the bootstrap
methodology doc, has since been retired — ADR 0010.)
...
(`spec/depth.md` — the faithful synthesis; `research/regrounding.md` — the design; ADR 0006
...
context cost, length signal, red flag, depth decision, strategic/tactical). (rebuild-spec, the
bootstrap methodology doc revised then, has since been retired — ADR 0010.) ADR 0006 supersedes in
part 0004 (the auto-refuse
...
- **(a) hypercore *using* context files for its own roles.** Durable, role-invariant grounding —
  ...
  is the live smell: it is a compressed copy of `spec/depth.md` pasted into a Python constant.
